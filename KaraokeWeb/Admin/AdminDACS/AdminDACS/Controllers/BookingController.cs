...
namespace AdminDACS.Controllers
{
    public class BookingController : Controller
    {
        ...
        public async Task<IActionResult> SubmitBooking(string FullName, string Phone, string Email, DateTime BookingTime, string RoomName)
        {
            ...
            {
                // Cập nhật thông tin khách hàng nếu số điện thoại đã tồn tại
        ...
        public IActionResult CancelBookingConfirmed(string roomId)
        {
            var room = _context.Rooms.Find(roomId);
            var booking = _context.bookings.FirstOrDefault(b => b.MaPhong == roomId && room.TinhTrang == true);
            if (booking == null)
            {
